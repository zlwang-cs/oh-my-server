...
def get_gpu_status(client):
    ...
    try:
        stdin, stdout, stderr = client.exec_command("nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits", timeout=10)
...
def check_gpu_server(hostname, ssh_config, timeout=10):
    ...
    try:
        ...
        client.connect(
            ...
            disabled_algorithms={'pubkeys': [host_config.get('PubkeyAcceptedKeyTypes')]} if host_config.get('PubkeyAcceptedKeyTypes') else None
        )
...
def print_gpu_status(all_gpu_info, only_free=False, brief=False):
    ...
    for hostname, gpus in servers.items():
        if brief:
            ...
            print(f"({Fore.GREEN if free_gpus > 0 else Style.DIM}Free: {free_gpus}{Style.RESET_ALL}, "
            ...
            print(f"{'GPU ID':6} {'GPU Name':20} {'Memory Usage':25} {'Mem %':8} {'Util %':8} {'Status':6}")
            ...
            for gpu in gpus:
                ...
                print(f"{color}{gpu['gpu_id']:6} {gpu['name'][:20]:20} {memory_str:25} {mem_ratio_str:8} {util_str:8} {status:6}{Style.RESET_ALL}")
